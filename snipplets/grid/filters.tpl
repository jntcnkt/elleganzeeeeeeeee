	...
	{% if has_applied_filters %}
		...
		<div class="mt-4 text-center">
			...
			{% for product_filter in product_filters %}
				...
				{% endfor %}
			{% endfor %}
			<a href="#" class="js-remove-all-filters btn-link btn-link-primary mt-1">{{ 'Borrar filtros' | translate }}</a> 
		</div>
	{% endif %}
{% else %}
							...
							<div class="col-12 col-sm-4 mb-5 mb-sm-3 p-0 p-sm-3" data-store="filters-group">
								...
								{% for value in product_filter.values %}
									{% if value.product_count > 0 %}
										...
										{% endif %}
											<label class="js-filter-checkbox {% if not value.selected %}js-apply-filter{% else %}js-remove-filter{% endif %} checkbox-container btn-filter {% if product_filter.type == 'color' %}btn-filter-color {% if value.color_type == 'insta_color' %}color-filter{% else %}color-filter-custom{% endif %}{% endif %}" data-filter-name="{{ product_filter.name|replace("'","%27") }}" data-filter-value="{{ value.name|replace("'","%27") }}">
