...
{% set payment_methods_config = product.payment_methods_config %}
    ...
    {% embed "snipplets/modal.tpl" with{modal_id: 'installments-modal', modal_class: 'installments', modal_position: 'bottom', modal_transition: 'slide', modal_header: true, modal_footer: true, modal_width: 'centered', modal_form_class: 'modal-fixed-wrapper', fixed_footer: true, fixed_header: true, modal_mobile_full_screen: 'true'} %}
        {% block modal_head %}
                        ...
                        {% if loop.last and custom_payment is not null %}
                            <div class="js-tab-panel tab-panel js-gw-tab-pane" id="installment_{{ custom_payment.code }}">
                                <div class="box">
                                    ...
                                    <h6 class="mb-3">{{ 'Cuando termines la compra vas a ver la información de pago en relación a esta opción.' | translate }}</h6>
                                    
                                    {% if custom_payment.discount > 0 %}
                                        <div class="mb-1">
                                            {% include "snipplets/svg/money-bill-wave.tpl" with {fa_custom_class : "fa-lg mr-2 svg-primary-text float-left"} %}
                                    ...
                                    <h3 class="mb-3 font-weight-normal">
                                        ...
                                        {% if custom_payment.discount > 0 %}
                                            ...
                                            <span class="js-installments-one-payment opacity-50 price-compare h4 font-weight-normal">{{ product.price | money }}</span><strong class="js-price-with-discount" data-payment-discount="{{ custom_payment.discount }}">{{ price_with_discount | money }}</strong> 
                                        {% else %} 
