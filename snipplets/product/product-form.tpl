...
<div class="price-container" data-store="product-price-{{ product.id }}">
    <span class="d-inline-block mr-3">
        <h3 class="js-price-display regular-price font-weight-bold" id="price_display" {% if not product.display_price %}style="display:none;"{% endif %}>{% if product.display_price %}{{ product.price | money }}{% endif %}</h3>
    </span>
...
{# Product form, includes: Variants, CTA and Shipping calculator #}
 ...
 <form id="product_form" class="js-product-form" method="post" action="{{ store.cart_url }}" data-store="product-form-{{ product.id }}">
    ...
    <div class="mb-3 full-width-container">
        ...
        <input type="submit" class="js-addtocart js-prod-submit-form btn btn-primary btn-block mb-2 {{ state }}" value="{{ texts[state] | translate }}" {% if state == 'nostock' %}disabled{% endif %} data-store="product-buy-button"/>
