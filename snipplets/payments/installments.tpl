{% if product %}
  ...
  {% if product.show_installments and product.display_price %}
    ...
    {% if product_detail and ( installments_info or custom_payment.discount > 0 ) %}
      <div data-toggle="#installments-modal" data-modal-url="modal-fullscreen-payments" class="js-fullscreen-modal-open js-product-payments-container js-modal-open row mb-4" {% if (not product.get_max_installments) and (not product.get_max_installments(false)) %}style="display: none;"{% endif %}>
    {% endif %}
     ...
     {% if product_can_show_installments %}
      ...
      <span class="js-max-installments-container js-max-installments {% if product_detail %}col-12  mb-2 col-md-auto{% else %}item-installments{% endif %}">
        {% if product_detail %}
        ...
        {% else %}
          {% if store.country != 'AR' or product_detail %}
            ...
            {% if max_installments_with_interests %}
              <span class="js-max-installments">{{ "<strong class='js-installment-amount installment-amount'>{1}</strong> cuotas de <strong class='js-installment-price installment-price'>{2}</strong>" | t(max_installments_with_interests.installment, max_installments_with_interests.installment_data.installment_value_cents | money) }}</span>
            {% else %}
    ...
    {% if product_detail and installments_info  %}
      <a id="btn-installments" class="btn-link btn-link-primary font-small col" {% if (not product.get_max_installments) and (not product.get_max_installments(false)) %}style="display: none;"{% endif %}>
        {% if not product_can_show_installments and custom_payment.discount == 0 %}{% include "snipplets/svg/money-bill-wave.tpl" with {fa_custom_class : "fa-lg mr-2 svg-primary-text float-left"} %}{% endif %}
        <span class="d-table text-inline">{{ "Ver más detalles" | translate }}</span>
      </a>
    </div>
    {% endif %}
