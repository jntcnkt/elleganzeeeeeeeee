...
#Label 
    ...
    // input_custom_label_text for custom label block    
#Prepend
...
<div class="form-group {{ input_group_custom_class }}">
    ...
    {% endif %}
    {% if input_custom_label_text %}
        <label {% if input_label_id %}id="{{ input_label_id }}"{% endif %} class="form-label {{ input_label_custom_class }}" {% if input_for %}for="{{ input_name }}"{% endif %}>
    ...
    {% else %}
        ...
        />
            {% if type_password %} 
                <a aria-label="{{ 'Ver contraseña' | translate }}" class="js-password-view form-toggle-eye">
                    <span class="js-eye-open" style="display: none;">
                        {% include "snipplets/svg/eye.tpl" with {svg_custom_class: "svg-inline--fa svg-icon-primary"} %}
                    </span>
                    <span class="js-eye-closed">
                        {% include "snipplets/svg/eye-closed.tpl" with {svg_custom_class: "svg-inline--fa svg-icon-primary"} %}
                    </span>
                </a>
            {% endif %}
    {% endif %}
    ...
    {% if input_help %}
    <div class="mt-4 text-right{% if settings.theme_variant == 'rounded' %} pr-3{% endif %}">
